...
- 转换命令返回非零退出码：读取结构化日志中的错误字段，按错误原因修复后重新执行。
...
- 文件不存在：确认 Step 2 是否成功执行，重新检查文件路径。
- 缺少 UTF-8 BOM：重新执行转换命令，确认文件由本脚本生成。
- 存在双 UTF-8 BOM：重新执行转换命令；若仍失败，检查文件开头内容。
- 存在裸 LF：重新执行转换命令；若仍失败，检查脚本换行归一化逻辑。
- 存在裸 CR：重新执行转换命令；若仍失败，检查脚本换行归一化逻辑。
- UTF-8 严格解码失败：重新执行转换命令；若仍失败，检查写出编码逻辑。
- 校验命令返回非零退出码：文件不视为有效产物，按校验日志修复后重新校验。
