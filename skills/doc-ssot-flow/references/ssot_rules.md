...
内容（必须遵守）：
- ID：使用 `task-001` 这种格式，三位数字递增（task-001/task-002/task-003...）；在 `doc/tasks.md` 中按编号升序排列。
- 简述：一句话描述任务目标（用户可理解），禁止写实现细节。
- 状态：仅允许 TODO/DOING/DONE。
- 文件：对应任务文件链接（如 `doc/task-001.md`）。
...
| ID | 简述 | 状态 | 文件 |
| --- | --- | --- | --- |
| task-001 | TODO | TODO | doc/task-001.md |
```
...
命名规则：任务 ID 为 `task-xxx` 则任务文件必须为 `doc/task-xxx.md`（编号一一对应）。
...
<!-- 非目标（Non-goals）：明确不做的范围与边界（含排除理由）；不适用写 N/A -->
### 非目标（Non-goals）
- <填写或 N/A>

## 约束（Constraints）

<!-- 约束：只描述用户可感知的状态与限制；必要时写清安全边界（只读源/操作区）与不可触碰目录/仓库 -->
- 用户可感知约束：<填写或 N/A>
- 业务/安全边界：<填写或 N/A>

## 设计（Design）

<!-- 设计：只记录最终方案与原因；过程性尝试放在 Issue 执行记录里；拆解 Issue 前先定义模块 API 或接口契约 -->
模块/接口契约：
- <填写或 N/A>
...
<!-- 记录方式：按 Issue 追加；Issue 结束以“结论”为准；计划/方案/验证方式变化写入“变更” -->
分析：
- <填写>
...
变更：
<!-- 若发生范围/方案/验证方式调整，记录原因与影响；否则写 N/A -->
- <填写或 N/A>
...
```mermaid
flowchart TD
  A[启动 Kickoff] --> B[初始化 Init]
  B --> C[任务定义 Task Definition]
  C --> D[Issue 循环 Per-Issue Loop]
  D --> E{本任务是否完成?}
  E -- 否 --> D
  E -- 是 --> F[闭环 Closeout]
  F --> G{是否触发回写触发器?}
  G -- 是 --> H[回写 doc/project.md]
  H --> I[更新 doc/tasks.md 状态]
  G -- 否 --> I[更新 doc/tasks.md 状态]
  I --> J{用户是否明确批准?}
  J -- 否 --> C
  J -- 是 --> K[任务标记 DONE]
```
...
1. 明确范围：确认本次任务 ID（`task-xxx`）与初始 Issue（通常从 `Issue-001` 开始编号）。
...
1. 新建/更新 `doc/task-xxx.md`，按模板填写：
   - 需求（Requirements）：按模板填写用户故事、验收标准（DoD）、非目标（Non-goals）。
   - 约束（Constraints）：按模板填写用户可感知约束、业务/安全边界（不适用写 N/A）。
   - 设计（Design）：按模板填写模块/接口契约（不适用写 N/A）。
...
2. 计划：写清将要做的修改与验证方式（以模板字段为准，不新增模板外字段）。
