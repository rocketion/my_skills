...
```mermaid
flowchart TD
  s31["7.1 启动（Kickoff）"] --> s32["7.2 初始化（Init）"]
  s32 --> s33["7.3 任务定义（Task Definition）"]
  s33 --> s34["7.4 Issue 启动与执行（Per-Issue Loop）"]
  s34 --> s35["7.5 执行检查并整理回写清单（Checklist）"]

  s35 --> d_need{"是否触发回写"}
  d_need -- 否 --> s38["7.8 Issue 结果整理（Issue Summary）"]
  d_need -- 是 --> d_backwrite{"用户确认：是否执行回写"}
  d_backwrite -- 否 --> s38
  d_backwrite -- 是 --> s36["7.6 回写决策整理（Backwrite Decision）"]
  s36 --> s37["7.7 回写流程（Backwrite Flow）"]
  s37 --> s38

  s38 --> s39["7.9 代码提交与索引更新（Commit）"]
  s39 --> d_next_issue{"用户确认：是否追加 Issue"}
  d_next_issue -- 是 --> s34
  d_next_issue -- 否 --> s310["7.10 任务终结（Task Closeout）"]
  s310 --> s311["7.11 最终代码提交（Final Commit）"]
  s311 --> done[任务标记完成]
```
...
2. 初始化或更新 `doc/project.md`：按 `references/project_template.md` 增量更新，不适用写 N/A。
3. 初始化或更新 `doc/tasks.md`：按 `references/tasks_template.md` 登记任务索引，只写 ID、简述、状态与文件路径。
...
2. 按 `references/task_template.md` 补齐“需求”“约束”“设计”章节。
3. 若 Issue 的范围、方案或验证方式变化，必须在对应 Issue 下记录变更原因与影响，并纳入 7.5 的回写清单；需要更新任务定义时在 7.7 回写流程中统一更新。

### 7.4 Issue 启动与执行（Per-Issue Loop）
...
4. 记录变更：若计划、方案或验证方式调整，追加写明原因与影响，并在 7.5 的回写清单中列为回写项。
5. 记录测试：在测试列表下记录范围、步骤、结果、成功日志与失败日志。
6. 记录结论：给出最终结果、原因与下一步边界，并写清本 Issue 的变更摘要与风险点。
7. 进入检查：转入 7.5 执行检查并整理回写清单。

### 7.5 执行检查并整理回写清单（Checklist）
...
8. 进入后续：若未触发回写，则直接进入 7.8；若触发回写，则等待用户确认后决定进入 7.6 或直接进入 7.8。

### 7.6 回写决策整理（Backwrite Decision）

定位：当 7.5 判定触发回写且用户确认执行回写时，先整理回写输入与执行边界，再进入正式回写流程。

执行步骤：
1. 记录确认结果：在当前 Issue 的“结论”中记录用户确认执行回写及其原因。
2. 明确回写边界：按 7.5 产出的回写清单，明确本轮需要回写的文档、条目与一致性校验点。
3. 进入回写：将整理结果作为 7.7 的直接输入。

### 7.7 回写流程（Backwrite Flow）
...
6. 回写摘要：整理本次回写的条目清单与关键变更点，作为 7.8 的输入。

### 7.8 Issue 结果整理（Issue Summary）
...
3. 提交输入：将上述汇总作为 7.9 的输入材料。

### 7.9 代码提交与索引更新（Commit）

定位：将 7.8 整理出的本轮结果先固化为一次可追溯提交；索引更新与状态更新必须与提交同步，避免后续追加 Issue 或结束 task 时出现记录漂移。

执行步骤：
1. 提交前核对：确认工作区改动与 7.8 的结果整理一致；确认回写闭环与回归记录（如适用）已完成且可追溯。
...
4. 执行提交：完成一次提交。
5. 进入流转：提交完成后，根据用户对“是否追加 Issue”的确认结果，追加则直接回到 7.4，不追加则进入 7.10。

### 7.10 任务终结（Task Closeout）
...
### 7.11 最终代码提交（Final Commit）

定位：将任务终结阶段的状态更新与收尾说明固化为最终一次提交，随后才允许将任务标记为完成。

执行步骤：
1. 提交前核对：确认 7.10 的任务终结说明、`doc/tasks.md` 状态更新与相关文档改动已经完整落盘。
